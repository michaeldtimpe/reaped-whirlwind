...
Compatible with existing RadarColorScale structure
"""
...
class RadarImageConverter:
    ...
    def _extract_colors_from_scale(self, scale: RadarColorScale):
        """Extract all colors and values from a RadarColorScale object."""
        colors = []
        values = []
        
        # The scale has colors stored in self.colors array
        # and we can get values using the scale's methods
        for color in scale.colors:
            colors.append(color)
            # Get the value for this color
            value = scale.find_closest_value(color)
            values.append(value)
        
        return np.array(colors), np.array(values)

    def _build_kdtrees(self):
        """Build KD-trees for ultra-fast color lookups."""
        # Extract colors and values
        refl_colors, refl_values = self._extract_colors_from_scale(self.reflectivity_scale)
        vel_colors, vel_values = self._extract_colors_from_scale(self.velocity_scale)
