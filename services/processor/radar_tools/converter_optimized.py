...
class RadarImageConverter:
    ...
    def _build_kdtrees(self):
        ...
        # Get all colors and values from scales
        refl_colors = []
        refl_values = []
        for color, value in self.reflectivity_scale.color_map.items():
            refl_colors.append(color)
            refl_values.append(value)
        
        vel_colors = []
        vel_values = []
        for color, value in self.velocity_scale.color_map.items():
            vel_colors.append(color)
            vel_values.append(value)
        
        # Build KD-trees (log(n) lookup instead of O(n)!)
        self.refl_tree = cKDTree(np.array(refl_colors))
        self.refl_values = np.array(refl_values)
        
        self.vel_tree = cKDTree(np.array(vel_colors))
        self.vel_values = np.array(vel_values)
